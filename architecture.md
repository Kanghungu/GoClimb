...
✅ Phase 5 - 안드로이드 앱 (구현 완료, 테스트 진행 중)
  ...
  ├── AppNavigation 확장 + Event/Profile/Stats 화면 추가
  └── GymDetailScreen / GymDetailViewModel 추가 + Calendar/Record 플로우 개선

✅ Phase 5-2 - 백엔드 추가 기능
  ├── 운동 기록 수정 API (PUT /api/records/{id})
  ├── FCM 토큰 등록/삭제 API (POST/DELETE /api/user/fcm-token)
  ├── FCM Token 엔티티 / Repository / Service 추가
  ├── UserController / SecurityConfig / UserService 연동 보강
