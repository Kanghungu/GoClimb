...
export default function ManagerColorPage() {
  ...
  const [form, setForm] = useState({ colorName: '', colorHex: '#000000', levelOrder: 1 })
  ...
  const createMutation = useMutation({
    mutationFn: () => api.post(`/gyms/${gymId}/colors`, form),
    onSuccess: () => { qc.invalidateQueries(['colors']); toast.success('추가됐습니다.'); setModal(false) },
    onError: (err) => toast.error(err.response?.data?.message || '오류'),
  })

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/gyms/${gymId}/colors/${id}`),
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => { setForm({ colorName: '', colorHex: '#000000', levelOrder: (colors.length + 1) }); setModal(true) }}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="number" min={1} value={form.levelOrder}
                  onChange={(e) => setForm({ ...form, levelOrder: parseInt(e.target.value) })}
