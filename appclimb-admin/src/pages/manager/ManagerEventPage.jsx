...
export default function ManagerEventPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      modal === 'create'
        ? api.post(`/gyms/${gymId}/events`, form)
        : api.put(`/gyms/${gymId}/events/${modal.id}`, form),
    onSuccess: () => { qc.invalidateQueries(['events']); toast.success('저장됐습니다.'); setModal(null) },
    onError: (err) => toast.error(err.response?.data?.message || '오류'),
  })

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/gyms/${gymId}/events/${id}`),
